...
import { Calendar, Star, Users, Clock, Lightbulb, CheckCircle, AlertCircle, Target } from "lucide-react"
import { format } from "date-fns"

interface MultigradeReflectionDisplayProps {
  reflection: any
  onEdit?: () => void
  onDelete?: () => void
}

export function MultigradeReflectionDisplay({ reflection, onEdit, onDelete }: MultigradeReflectionDisplayProps) {
  ...
  const getEffectivenessColor = (effectiveness: string) => {
    switch (effectiveness?.toLowerCase()) {
      case "very effective": return "bg-green-100 text-green-800"
      case "effective": return "bg-blue-100 text-blue-800"
      case "somewhat effective": return "bg-yellow-100 text-yellow-800"
      case "not effective": return "bg-red-100 text-red-800"
      default: return "bg-gray-100 text-gray-800"
    }
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <CardTitle className="text-lg">Multigrade Lesson Reflection</CardTitle>
              <CardDescription>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold">Multigrade-Specific Assessment</h3>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {reflection.grade_level_effectiveness && (
              <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
                <div className="flex-shrink-0 mt-1">
                  <Target className="h-5 w-5 text-blue-600" />
                </div>
                <div className="flex-1">
                  <p className="font-medium text-sm">Multigrade Approach Effectiveness</p>
                  <Badge className={`mt-1 ${getEffectivenessColor(reflection.grade_level_effectiveness)}`}>
                    {reflection.grade_level_effectiveness}
                  </Badge>
                </div>
              </div>
            ...
            {renderChecklistItem(
              "Did peer learning work well between grade levels?",
              reflection.peer_learning_success
            )}
            
            {renderChecklistItem(
              "Were differentiation strategies effective across grade levels?",
              reflection.differentiation_success
            )}
          </div>
          
          {reflection.cross_grade_collaboration && (
            <div className="space-y-2">
              <h4 className="font-medium text-sm">Cross-Grade Collaboration</h4>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {renderTextResponse(
              "Evidence Supporting Student Learning",
              reflection.evidence_support
            )}
            {renderTextResponse(
              "Students Not Meeting Objectives",
